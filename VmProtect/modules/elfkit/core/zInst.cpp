...
zInst::zAsmDomain zInst::classifyArm64Domain(unsigned int instructionId) {  // 处理阶段入口：进入该函数或代码块的主流程。
    ...
    switch (instructionId) {  // 分发表入口：按标签分发到对应语义实现。
        ...
        case ARM64_INS_CSNEG: // 指令分支：ARM64_INS_CSNEG，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CCMP: // 指令分支：ARM64_INS_CCMP，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CCMN: // 指令分支：ARM64_INS_CCMN，在此分支内完成等价 VM 语义映射。
        ...
        case ARM64_INS_ALIAS_CNEG: // 指令分支：ARM64_INS_ALIAS_CNEG，在此分支内完成等价 VM 语义映射。
        ...
        case ARM64_INS_LD4: // 指令分支：ARM64_INS_LD4，在此分支内完成等价 VM 语义映射。
        ...
        case ARM64_INS_FMOV: // 指令分支：ARM64_INS_FMOV，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_FMOV: // 指令分支：ARM64_INS_ALIAS_FMOV，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_INS: // 指令分支：ARM64_INS_INS，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_INSR: // 指令分支：ARM64_INS_INSR，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_MOVA: // 指令分支：ARM64_INS_MOVA，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_UMOV: // 指令分支：ARM64_INS_UMOV，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_SMOV: // 指令分支：ARM64_INS_SMOV，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_DUP: // 指令分支：ARM64_INS_DUP，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FCMP: // 指令分支：ARM64_INS_FCMP，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FCVT: // 指令分支：ARM64_INS_FCVT，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FCVTAS: // 指令分支：ARM64_INS_FCVTAS，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CMEQ: // 指令分支：ARM64_INS_CMEQ，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CMHI: // 指令分支：ARM64_INS_CMHI，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CMHS: // 指令分支：ARM64_INS_CMHS，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CMGT: // 指令分支：ARM64_INS_CMGT，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_CMLT: // 指令分支：ARM64_INS_CMLT，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_BIT: // 指令分支：ARM64_INS_BIT，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_BSL: // 指令分支：ARM64_INS_BSL，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_SHL: // 指令分支：ARM64_INS_SHL，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_SHLL: // 指令分支：ARM64_INS_SHLL，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_SHLL2: // 指令分支：ARM64_INS_SHLL2，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_USHR: // 指令分支：ARM64_INS_USHR，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_USHLL: // 指令分支：ARM64_INS_USHLL，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_USHLL2: // 指令分支：ARM64_INS_USHLL2，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_SHRN: // 指令分支：ARM64_INS_SHRN，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_XTN: // 指令分支：ARM64_INS_XTN，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FNEG: // 指令分支：ARM64_INS_FNEG，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FDIV: // 指令分支：ARM64_INS_FDIV，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FADD: // 指令分支：ARM64_INS_FADD，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FMUL: // 指令分支：ARM64_INS_FMUL，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_SCVTF: // 指令分支：ARM64_INS_SCVTF，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_FCVTZS: // 指令分支：ARM64_INS_FCVTZS，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_UCVTF: // 指令分支：ARM64_INS_UCVTF，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_NOT: // 指令分支：ARM64_INS_NOT，在此分支内完成等价 VM 语义映射。
        ...
        case ARM64_INS_ALIAS_TST: // 指令分支：ARM64_INS_ALIAS_TST，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ORR: // 指令分支：ARM64_INS_ORR，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_ORR: // 指令分支：ARM64_INS_ALIAS_ORR，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ORN: // 指令分支：ARM64_INS_ORN，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_MVN: // 指令分支：ARM64_INS_ALIAS_MVN，在此分支内完成等价 VM 语义映射。
        ...
        case ARM64_INS_BFM: // 指令分支：ARM64_INS_BFM，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_BFI: // 指令分支：ARM64_INS_ALIAS_BFI，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_LSR: // 指令分支：ARM64_INS_ALIAS_LSR，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_ASR: // 指令分支：ARM64_INS_ALIAS_ASR，在此分支内完成等价 VM 语义映射。
        ...
        case ARM64_INS_NEG: // 指令分支：ARM64_INS_NEG，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_NEG: // 指令分支：ARM64_INS_ALIAS_NEG，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ADD: // 指令分支：ARM64_INS_ADD，在此分支内完成等价 VM 语义映射。
        case ARM64_INS_ALIAS_CMN: // 指令分支：ARM64_INS_ALIAS_CMN，在此分支内完成等价 VM 语义映射。
...
static zInstAsmUnencodedBytecode buildUnencodedByCapstone(csh handle, const uint8_t* code, size_t size, uint64_t baseAddr) {
    ...
    for (size_t branchIndex = 0; branchIndex < branch_id_list.size(); ++branchIndex) {
        ...
        if (targetPcIt == addr_to_pc.end()) {
            ...
            // 允许“函数外分支目标”（例如尾调用/跳板跳转到未加固函数）：
            // 这类目标不属于当前函数本地地址映射范围，不再作为翻译失败处理。
            // 这里把目标映射到函数尾 PC（终止位点），确保分支表下标与数量一致。
            // 注意：仅对“函数外目标”放宽；函数内目标仍保持严格校验，避免掩盖真实翻译缺陷。
            if (unencoded.instByAddress.empty() ||
                arm_addr < min_inst_addr ||
                arm_addr > max_inst_addr) {
                unencoded.branchWords.push_back(pc);
                continue;
            }
