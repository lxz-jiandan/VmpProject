/*
 * [VMP_FLOW_NOTE] zFunction 核心实现文件。
 * - 本文件聚合 zFunction 的门面访问、指令展示缓存、未编码缓存翻译三类职责。
 * - 目标是把分散在 zFunctionView/zFunctionFacade 的实现收拢，降低跨文件跳转成本。
 * - 具体 AArch64->VM 指令映射仍由 zInstAsm/zInst* 完成，本文件只做编排与缓存管理。
 *
 * [VMP_FLOW_LAYOUT] 文件内部分区：
 * 1) 基础门面与访问器
 * 2) 指令展示缓存（inst_view）
 * 3) 未编码翻译缓存（unencoded）
 * 4) dump 导出辅助工具（匿名命名空间）
 * 5) dump 导出入口
 */
#include "zFunction.h"
#include "zInst.h"
...
static std::string formatOpcodeList(const std::vector<uint32_t>& opcodeList, bool trailingComma = true) {
    ...
    for (size_t wordIndex = 0; wordIndex < opcodeList.size(); ++wordIndex) {
        ...
        // 非最后一个补逗号。
        if (wordIndex + 1 < opcodeList.size()) {
            result += ",";
        }
    }
    // 兼容历史格式：行尾可选补逗号。
    if (trailingComma && !opcodeList.empty()) {
        result += ",";
    }
    return result;
}
