...
bool validateRegionAllowEmpty(const uint32_t headerSize,
...
// 校验区域：要求 used <= cap。
bool validateUsedRegion(const uint32_t headerSize,
                        const uint32_t totalSize,
                        const uint32_t off,
                        const uint32_t cap,
                        const uint32_t used,
...
bool writeRegionPadded(std::vector<uint8_t>* bytes,
    ...
    // 计算绝对写入起始偏移。
    const uint64_t absOffsetU64 = baseOff + static_cast<uint64_t>(off);
    // 计算绝对写入结束偏移。
