#ifndef VMPROTECT_PATCHBAY_DONOR_H
#define VMPROTECT_PATCHBAY_DONOR_H

// 引入字符串类型。
#include <string>

// patchbay donor 导出流程的输入参数。
struct zPatchbayDonorRequest {
    // 输入 vmengine so（通常是 embed 后的临时 so）。
    std::string inputSoPath;
    // donor so（提供导出符号集合与 st_value key）。
...
// donor 流程状态码（用于结构化错误定位）。
enum class zPatchbayDonorStatus {
    // 成功完成。
    ok = 0,
    // 参数缺失或不合法。
    invalidInput,
...
// donor 流程执行结果。
struct zPatchbayDonorResult {
    // 执行状态。
    zPatchbayDonorStatus status = zPatchbayDonorStatus::ok;
    // 兼容 CLI 的退出码语义（0/1/2/3）。
    int exitCode = 0;
    // 详细错误文本（成功时可为空）。
    std::string error;
    // donor 原始导出数量。
    size_t donorExportCount = 0;
    // input 原始导出数量。
...
#endif // VMPROTECT_PATCHBAY_DONOR_H
