...
def run_cmd(cmd, cwd=None, env=None, check=True):
    ...
    print(f"$ {' '.join(cmd)}")
    # 执行命令并抓取 stdout/stderr，统一转文本。
    proc = subprocess.run(
        # 命令参数数组，避免 shell 拼接转义问题。
        cmd,
        # 工作目录（None 表示沿用当前进程目录）。
        cwd=cwd,
        # 进程环境变量（None 表示继承当前环境）。
        env=env,
        # 捕获输出，便于统一打印与失败定位。
        capture_output=True,
        # 按文本模式读取输出，避免手工 decode。
        text=True,
        # 非法字符替换，避免编码异常中断脚本。
        errors="replace",
    )
    ...
    # 返回 CompletedProcess 供调用方读取返回码和输出。
    return proc
...
def parse_local_properties(path: Path):
    # 默认未知 sdk.dir。
    sdk_dir = None
    # local.properties 不存在则直接返回 None。
    if not path.exists():
        return sdk_dir
    # 逐行解析，忽略空行和注释。
...
def patch_vmengine_symbols_patchbay(
    ...
    for target_so in targets:
        ...
        validate_android_elf_layout(patched_so)
...
def restore_vmengine_outputs(staged_for_install):
...
def main():
    ...
    if args.patch_vmengine_symbols:
        ...
        if not donor.is_absolute():
            donor = root / donor
        # 执行 patch 流程。
        staged_for_install = patch_vmengine_symbols_patchbay(
            project_root=root,
        ...
        try:
            ...
            restore_vmengine_outputs(staged_for_install)
    else:
